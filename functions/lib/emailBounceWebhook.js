...
exports.brevoEmailWebhook = void 0;
 ...
 */
exports.brevoEmailWebhook = (0, https_1.onRequest)({ region: "us-central1", cors: false }, async (req, res) => {
    ...
    try {
        ...
        let matchedLog = null;
        if (messageId) {
            const snap = await emailShared_1.db.collection("emailLogs").where("messageId", "==", messageId).limit(1).get();
            if (!snap.empty)
                matchedLog = snap.docs[0];
        }
        const matchedData = matchedLog?.data() || {};
        if (HARD_BOUNCE_EVENTS.has(event)) {
            if (matchedLog) {
                await matchedLog.ref.update({ status: "bounced", error: reason || event });
            }
            await emailShared_1.db.collection("emailSuppressions").doc(email).set({
                to: email,
...
//# sourceMappingURL=emailBounceWebhook.js.map