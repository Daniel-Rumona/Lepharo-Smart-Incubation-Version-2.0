...
exports.sendComplianceReminderEmail = exports.sendApplicationReceivedEmail = exports.resendAccountCreationEmails = exports.sendAdminTestEmail = exports.queueSystemStatusBroadcast = exports.onMailCampaignQueued = void 0;
...
// -------------------- Templates --------------------
function basicShell(title, bodyHtml) {
    return `
...
function buildSystemNoticeEmail(args) {
    ...
    const html = basicShell(config.title, `
      <p>Hi ${greetingName},</p>
...
function buildAccountCreationEmail(args) {
    const subject = 'Welcome to Lepharo Smart Incubation';
    const html = basicShell('Account created', `
    <p>Hi ${args.name || 'there'},</p>
...
function buildApplicationReceivedEmail(args) {
    const subject = 'We received your application';
    const html = basicShell('Application received', `
    <p>Hi ${args.name || 'there'},</p>
...
function buildApplicationAcceptedEmail(args) {
    const subject = 'Application accepted';
    const html = basicShell('Accepted 🎉', `
    <p>Hi ${args.name || 'there'},</p>
...
function buildApplicationRejectedEmail(args) {
    const subject = 'Application update';
    const html = basicShell('Application update', `
    <p>Hi ${args.name || 'there'},</p>
...
function resolveTemplate(campaign, recipient) {
    ...
    switch (campaign.emailType) {
        ...
        case 'SYSTEM_RESOLVED': {
            const kind = campaign.emailType === 'SYSTEM_OUTAGE'
                ? 'outage'
...
function assertCampaignValid(c) {
    if (!c.name)
        throw new Error('name missing');
    if (!c.emailType)
        throw new Error('emailType missing');
    if (!c.recipientType)
        throw new Error('recipientType missing');
    if (!c.status)
        throw new Error('status missing');
    const p = c.payload || {};
    if (c.emailType === 'WELCOME' && !p.loginUrl)
        throw new Error('WELCOME requires payload.loginUrl');
    if (c.emailType === 'CUSTOM') {
        const ok = (c.custom?.html && c.custom?.subject) || c.subjectOverride;
        if (!ok)
            throw new Error('CUSTOM requires custom.subject + custom.html (or subjectOverride at least)');
    }
}
...
function parseEmailInputs(...values) {
...
function toAccountCreationTargets(docs, manualTargets, max) {
    return [
        ...docs.map((docSnap) => ({
            id: docSnap.id,
            ref: docSnap.ref,
...
async function requireEmailAdmin(req) {
    if (!req.auth)
    ...
    if (!hasAdminClaim && !ADMIN_EMAIL_ROLES.has(role)) {
        throw new https_1.HttpsError('permission-denied', 'Only system administrators can manage email delivery.');
    }
    return {
        uid: req.auth.uid,
...
async function markUserEmailStatus(userRef, status, error) {
    const patch = {
...
async function collectUsersForAccountCreation(input) {
    ...
    if (inputEmails.length) {
        ...
        inputEmails
            ...
            .forEach((email) => manualTargets.set(`manual:${email}`, {
            id: `manual:${email}`,
            data: { email },
            email
        }));
    ...
    if (!input.createdFrom || !input.createdTo) {
        if (manualTargets.size || seen.size)
            return toAccountCreationTargets([...seen.values()], manualTargets, max);
        throw new https_1.HttpsError('invalid-argument', 'Select users, choose roles, paste emails, or provide a creation date range.');
    }
    const from = new Date(`${input.createdFrom}T00:00:00.000Z`);
    const to = new Date(`${input.createdTo}T23:59:59.999Z`);
    if (Number.isNaN(from.getTime()) || Number.isNaN(to.getTime())) {
        throw new https_1.HttpsError('invalid-argument', 'Invalid creation date range.');
    }
    let q = db.collection('users');
    const snap = await q.limit(1000).get();
...
exports.onMailCampaignQueued = (0, firestore_1.onDocumentWritten)({
    region: 'us-central1',
    ...
    try {
        ...
        await campaignRef.set({ status: 'sending', updatedAt: admin.firestore.FieldValue.serverTimestamp(), sentCount: 0, failCount: 0 }, { merge: true });
        const recipientsSnap = await campaignRef.collection('recipients').get();
        await campaignRef.set({ totalRecipients: recipientsSnap.size, updatedAt: admin.firestore.FieldValue.serverTimestamp() }, { merge: true });
        let sent = 0;
        let failed = 0;
        ...
        for (const docSnap of recipientsSnap.docs) {
            ...
            try {
                ...
                await (0, mailer_1.sendEmail)({ to, subject: tpl.subject, html: tpl.html, text: tpl.text });
                sent++;
            ...
            await campaignRef.set({ sentCount: sent, failCount: failed, updatedAt: admin.firestore.FieldValue.serverTimestamp() }, { merge: true });
        }
        ...
        if (after.systemNotice && copyRecipients.length) {
            ...
            try {
                ...
                if (!copyTo)
                    throw new Error('Missing MAIL_FROM_EMAIL or SMTP_USER for broadcast copy.');
                await (0, mailer_1.sendEmail)({ to: copyTo, cc: copyRecipients, subject: tpl.subject, html: tpl.html, text: tpl.text });
                await campaignRef.collection('mailLogs').add({
                    campaignId,
                    emailType: after.emailType,
                ...
                firebase_functions_1.logger.error('systemStatusBroadcast.copyFailed', { campaignId, error });
            }
        }
        await campaignRef.set({ status: 'completed', updatedAt: admin.firestore.FieldValue.serverTimestamp() }, { merge: true });
    }
    catch (err) {
        const msg = String(err?.message || err);
        firebase_functions_1.logger.error('mailCampaign.failed', { campaignId, error: msg });
        await campaignRef.set({ status: 'failed', updatedAt: admin.firestore.FieldValue.serverTimestamp(), lastError: msg }, { merge: true });
    }
});
exports.queueSystemStatusBroadcast = (0, https_1.onCall)({ region: 'us-central1', cors: CALLABLE_CORS, timeoutSeconds: 120, memory: '512MiB' }, async (req) => {
    const actor = await requireEmailAdmin(req);
    if (req.data?.confirmation !== 'SEND TO ALL USERS') {
        throw new https_1.HttpsError('failed-precondition', 'Confirm that this notice must be sent to all users.');
    }
    const kind = safeLower(req.data?.kind);
    if (!Object.prototype.hasOwnProperty.call(SYSTEM_NOTICE_CONFIG, kind)) {
        throw new https_1.HttpsError('invalid-argument', 'Choose outage, upgrade, or resolved.');
    }
    ...
    if (message.length < 10 || message.length > 2000) {
        throw new https_1.HttpsError('invalid-argument', 'The notice message must be between 10 and 2,000 characters.');
    }
    if (subject.length < 5 || subject.length > 160) {
        throw new https_1.HttpsError('invalid-argument', 'The subject must be between 5 and 160 characters.');
    }
    ...
    if (startsAt && endsAt && new Date(endsAt).getTime() <= new Date(startsAt).getTime()) {
        throw new https_1.HttpsError('invalid-argument', 'Expected completion must be after the start time.');
    }
    const [usersSnap, suppressionsSnap] = await Promise.all([
        db.collection('users').get(),
        db.collection('emailSuppressions').get()
    ]);
    const suppressedEmails = new Set(suppressionsSnap.docs.map((docSnap) => safeLower(docSnap.data()?.to || docSnap.data()?.email)).filter(Boolean));
    ...
    if (recipients.size === 0) {
        throw new https_1.HttpsError('failed-precondition', 'No deliverable user email addresses were found.');
    }
    const emailType = kind === 'outage'
        ? 'SYSTEM_OUTAGE'
        : kind === 'upgrade'
            ? 'SYSTEM_UPGRADE'
            : 'SYSTEM_RESOLVED';
    ...
    try {
        ...
        firebase_functions_1.logger.error('systemStatusBroadcast.queueFailed', { campaignId: campaignRef.id, error });
        throw new https_1.HttpsError('internal', 'The system notice could not be queued.');
    }
    return {
        ok: true,
...
exports.sendAdminTestEmail = (0, https_1.onCall)({ region: 'us-central1', cors: CALLABLE_CORS }, async (req) => {
    const actor = await requireEmailAdmin(req);
    const to = safeLower(req.data?.to || actor.email);
    if (!to || !EMAIL_RX.test(to))
        throw new https_1.HttpsError('invalid-argument', 'Enter a valid test recipient email.');
    const subject = 'Smart Incubation email test';
    const html = basicShell('Email test', `
    ...
    try {
        const info = await (0, mailer_1.sendEmail)({ to, subject, html, text });
        await writeEmailLog({
            type: 'TEST',
            to,
            subject,
        ...
        throw new https_1.HttpsError('internal', error);
    }
});
exports.resendAccountCreationEmails = (0, https_1.onCall)({ region: 'us-central1', cors: CALLABLE_CORS }, async (req) => {
    const actor = await requireEmailAdmin(req);
    ...
    for (const target of targets) {
        ...
        if (!to || !EMAIL_RX.test(to)) {
            ...
            if (target.ref)
                await markUserEmailStatus(target.ref, 'skipped', error);
            await writeEmailLog({
        ...
        try {
            ...
            if (target.ref)
                await markUserEmailStatus(target.ref, 'ok');
            await writeEmailLog({
                type: 'ACCOUNT_CREATION',
                to,
                subject: tpl.subject,
            ...
            if (target.ref)
                await markUserEmailStatus(target.ref, 'bounced', error);
            await writeEmailLog({
                type: 'ACCOUNT_CREATION',
                to,
                subject: tpl.subject,
...
exports.sendApplicationReceivedEmail = (0, https_1.onCall)({ region: 'us-central1', cors: CALLABLE_CORS }, async (req) => {
    if (!req.auth)
        throw new https_1.HttpsError('unauthenticated', 'Login required.');
    ...
    if (!to || !EMAIL_RX.test(to)) {
        throw new https_1.HttpsError('invalid-argument', 'A valid applicant email is required.');
    }
    const tpl = buildApplicationReceivedEmail({ name, programName });
    try {
        const info = await (0, mailer_1.sendEmail)({ to, subject: tpl.subject, html: tpl.html, text: tpl.text });
        await writeEmailLog({
        ...
        throw new https_1.HttpsError('internal', 'Application confirmation email failed.');
    }
});
exports.sendComplianceReminderEmail = (0, https_1.onCall)({ region: 'us-central1', cors: CALLABLE_CORS }, async (req) => {
    if (!req.auth)
        throw new https_1.HttpsError('unauthenticated', 'Login required.');
    const to = safeLower(req.data?.email);
    const name = String(req.data?.name || 'there').trim();
    ...
    const issues = Array.isArray(req.data?.issues)
        ? req.data.issues.map((value) => String(value || '').trim()).filter(Boolean).slice(0, 50)
        : [];
    if (!to || !EMAIL_RX.test(to)) {
        throw new https_1.HttpsError('invalid-argument', 'A valid recipient email is required.');
    }
    if (!issues.length) {
        throw new https_1.HttpsError('invalid-argument', 'At least one outstanding compliance item is required.');
    }
    const subject = `Action required: outstanding details for ${programName}`;
    const issueList = issues.map((issue) => `<li>${issue.replace(/[&<>"']/g, (char) => ({
        '&': '&amp;',
        '<': '&lt;',
        '>': '&gt;',
        '"': '&quot;',
    ...
    const html = basicShell('Outstanding information required', `<p>Hi ${name},</p>
    ...
    try {
        const { messageId } = await (0, brevoClient_1.sendViaBrevo)({ to, subject, html, text, tags: ['compliance-reminder'] });
        ...
        throw new https_1.HttpsError('internal', 'Compliance reminder email failed.');
    }
});
//# sourceMappingURL=emailCampaigns.js.map