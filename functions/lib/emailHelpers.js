...
exports.buildSmmeDpReminderEmail = exports.buildSmmeDpSectionSubmittedEmail = exports.coreChanged = exports.buildICS = exports.buildAppointmentEmailHTML = exports.buildRejectionEmail = exports.buildAcceptanceEmail = exports.buildWelcomeEmail = exports.roleDisplayName = void 0;
// ── Email helpers ─────────────────────────────────────────────────────────────
...
function roleDisplayName(backendRole) {
    const map = {
        projectadmin: "Center Coordinator",
    };
    return map[backendRole] ?? titleCase(backendRole);
}
exports.roleDisplayName = roleDisplayName;
...
function buildAcceptanceEmail(opts) {
...
exports.buildAcceptanceEmail = buildAcceptanceEmail;
...
exports.buildRejectionEmail = buildRejectionEmail;
function buildAppointmentEmailHTML(opts) {
    const { appt, start, end, participantName, participantPhone, type } = opts;
    const badge = type === "created" ? { text: "New Appointment", bg: "#1677ff" } :
...
exports.buildAppointmentEmailHTML = buildAppointmentEmailHTML;
...
exports.coreChanged = coreChanged;
 ...
 */
function buildSmmeDpSectionSubmittedEmail(opts) {
    ...
    const intvLine = typeof opts.interventionCount === 'number'
...
exports.buildSmmeDpSectionSubmittedEmail = buildSmmeDpSectionSubmittedEmail;
/**
 * Email B: manual reminder ("Remind SME" button).
 * Truthful wording: "Reminder from X department" + progress.
 */
function buildSmmeDpReminderEmail(opts) {
    const toName = safe(opts.name || 'there');
    const fromDept = safe(opts.reminderFromDeptName);
    const programName = safe(opts.programName || '');
    const url = opts.confirmUrl;
    const prog = progressLine(opts.deptConfirmed, opts.deptTotal);
    const actionLine = opts.reminderType === 'final_confirmation_pending'
...
exports.buildSmmeDpReminderEmail = buildSmmeDpReminderEmail;
